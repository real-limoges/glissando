...
///
/// Layout matches glissando's own: per parameter, `[intercept, slope?]`, in the
/// order `specs` is given (callers keep that aligned with
/// `family.parameters()`).
...
/// Total log-likelihood at `beta`. The only glissando code this oracle touches.
...
fn maximize<D: Distribution + ?Sized>(
    ...
    for _ in 0..500 {
        ...
        // Guard against it: fall back to steepest ascent whenever the direction
        // is not an ascent direction to first order.
...
fn default_link_fits_reach_the_mle() {
    ...
    // Gaussian / identity + log σ. Two parameters, so this also exercises how
    // the oracle handles a nuisance parameter.
    {
